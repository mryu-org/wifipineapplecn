...
class Recon extends SystemModule
{
    ...
    {
        exec("pineap /tmp/pineap.conf get_status 2> /dev/null", $statusOutput);
        $statusOutput = explode(" ", $statusOutput[0]);
        $scanID = preg_replace('/[^0-9]/', '', $statusOutput[0]);
    ...
    {
        exec("pineap /etc/pineap.conf get_status", $statusOutput);

        if ($statusOutput[0] == "No scan in progress") {
            $this->stopScan();
            $this->response = array("completed" => true);
        } else if ($statusOutput[0] == "PineAP is not running") {
            $this->stopScan();
            $this->response = array("completed" => true, "error" => "The PineAP Daemon must be running.");
        } else if (is_string($statusOutput[0]) && substr($statusOutput[0], 0, strlen("scanid=")) === "scanid=") {
            list($nscanID, $scanStatus, $liveScan) = sscanf($statusOutput[0], "scanid=%d status=%d%% live=%d");
            $this->response = array(
                "completed" => false,
                "scanID" => $nscanID,
                "scanPercent" => $scanStatus,
                "continuous" => $liveScan,
                "live" => $this->wsRunning()
            );
        } else {
            $this->stopScan();
            $this->response = array("completed" => true, "error" => "Unknown error", "debug" => $statusOutput);
        }
    }
    ...
    private function getScanObject($scanID)
    {
        ...
        foreach ($aps as $ap_row) {
            ...
            $data[$scanID]['aps'][$ap_row['bssid']]['last_seen'] = $this->utcToPineapple($ap_row['last_seen']);
